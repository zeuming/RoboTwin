echo "apple_cabinet_storage" | python main.py
echo "block_hammer_beat" | python main.py
echo "block_handover" | python main.py
echo "block_sweep" | python main.py
echo "dual_bottles_pick" | python main.py
echo "empty_cup_place" | python main.py
# new task
echo "block_hammer_beat_hard" | python main.py
echo "blocks_stack" | python main.py
echo "diverse_bottles_pick" | python main.py
echo "mug_hanging" | python main.py
echo "put_multiobj_into_box" | python main.py
echo "shoe_place" | python main.py
echo "shoes_place_hard" | python main.py
