...
class gpt_container_place(container_place):
    def play_once(self):
        ...
        # Close the gripper to grasp the container
        close_gripper(pos=-0.01)  # Tighten the gripper to ensure a firm grasp
        ...
        place_pre_pose = self.get_grasp_pose_from_goal_point_and_direction(container, container_data, endpose_tag=endpose_tag, target_point=plate_target_pose, target_approach_direction=self.world_direction_dic['top_down'], pre_dis=0.09)
        place_target_pose = self.get_grasp_pose_from_goal_point_and_direction(container, container_data, endpose_tag=endpose_tag, target_point=plate_target_pose, target_approach_direction=self.world_direction_dic['top_down'], pre_dis=0)
        
        # Move the arm to the pre-place pose
        arm_move_to_pose(place_pre_pose)
        
        # Move the arm to the target place pose
        arm_move_to_pose(place_target_pose)
